...
copy_to_remote_pat="rsync -e 'ssh -o UserKnownHostsFile=$keyscan -avz <local_path> <remote_user>@<remote_ip>:<remote_path>"
copy_from_remote_pat="rsync -e 'ssh -o UserKnownHostsFile=$keyscan -avz <remote_user>@<remote_ip>:<remote_path> <local_path>"
run_remote_pat="$run_server_remote_pat"
exe_remote_pat="ssh -o UserKnownHostsFile=$keyscan <remote_user>@<remote_ip> bash"
